...
uint8_t *encode_DNA(const char *DNA_seq)
{
    ...
    uint8_t *encoded_DNA = (uint8_t *)calloc((len_DNA + BYTE_SIZE/BASE_BITS - 1) / 4, sizeof(uint8_t));
    if (encoded_DNA == NULL)
    {
        return NULL;
    }
    ...
    hash_table *ht_gene)
{
    ...
    sql = "CREATE TABLE umi (cell_index INTEGER, gene_index INTEGER, encoded_DNA TEXT);";
            ...
            if (temp1 != NULL)
            {
                ...
                if (UMI_quality == 25)
                {
                    ...
                    if (temp2 != NULL)
                    {
                        ...
                        char *encoded_DNA = encode_UMI(UMI);
                        sqlite3_bind_int(stmt, 1, cell_index);
                        sqlite3_bind_int(stmt, 2, gene_index);
                        sqlite3_bind_text(stmt, 3, encoded_DNA, strlen(encoded_DNA), SQLITE_STATIC);
                        free(encoded_DNA);
