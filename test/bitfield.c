...
bool compress_DNA_seqence(const char* sequence, uint8_t *compressed) {
    // calculate the length of compressed sequence
    size_t compressed_length = (strlen(sequence) * BASE_BITS + BYTE_SIZE - 1) / BYTE_SIZE;
    if (compressed == NULL) {
        return false;
    }
    int bit_index = BYTE_SIZE - BASE_BITS;
...
int main() {
    ...
    uint8_t *encoded_umi = (uint8_t *)calloc(3, sizeof(uint8_t));

    compress_DNA_seqence("ACGTACGTTT", encoded_umi);

    uint8_t *encoded_umi2 = calloc(3, sizeof(uint8_t));

    compress_DNA_seqence("AACCGGTTAC", encoded_umi2 );

    encoded_umi = realloc(encoded_umi, 6*sizeof(uint8_t));

    memcpy(encoded_umi + 3, encoded_umi2, 3*sizeof(uint8_t));

    printf("encoded_umi: %02X%02X%02X%02X%02X%02X\n", *encoded_umi, *(encoded_umi + 1), *(encoded_umi + 2), *(encoded_umi + 3), *(encoded_umi + 4), *(encoded_umi + 5));
