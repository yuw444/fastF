...
ENCODED_FUNC_INT(umi_bytes, encode_UMI);

// struct entry of hash table gene_UMI
typedef struct _obj_gene_UMI
{
    size_t n_UMIs;
    umi_bytes *UMIs;
}obj_gene_UMI;

bool hash_table_inser_UMI(hash_table *ht, const char *key, void *obj)
{
    ...
    char *output_file)
{
    ...
    size_t total_cells = ht_cell->size;
    const int tablesize_gene_UMI = 1000;

    hash_table **ht_gene_UMI = calloc(total_cells, sizeof(hash_table *));
    for (size_t i = 0; i < total_cells; i++)
    {
        ht_gene_UMI[i] = hash_table_create(tablesize_gene_UMI, hash, free);
    }
    ...
    while (sam_read1(bam_reader, bam_header, bam_record) >= 0)
    {
        ...
        if (cb != NULL)
        {
            ...
            if (UMI_quality == 25)
            {
                ...
                void *temp3 = hash_table_lookup(ht_gene_UMI[cell_index], gene_ID);
                if (temp3 == NULL)
                {
                    hash_table_insert(ht_gene_UMI[cell_index], gene_ID, UMI);
                }
                else
                {
                    obj_gene_UMI *gene_UMI = (obj_gene_UMI *) temp3;
                    gene_UMI->n_UMIs++;
                    gene_UMI->UMIs = realloc(gene_UMI->UMIs, gene_UMI->n_UMIs * sizeof(char *));
                    encode_UMI(UMI, gene_UMI->UMIs + gene_UMI->n_UMIs - 1);
                }
            }
        }
    }
